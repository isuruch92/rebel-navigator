...
export class MapService {
  ...
  private handleMapClick(event: any): void {
    if (this.map instanceof Map) {
      ...
      if (clickedCoords) {
        const [longitude, latitude] = transform(clickedCoords, 'EPSG:3857', 'EPSG:4326');
  ...
  public flashMarker(feature: Feature, markerLayer: VectorLayer<any>, color: Color): void {
    const start = Date.now();
    const flashGeom = feature.getGeometry()!.clone();
    const listenerKey = markerLayer.on('postrender', (event) => this.animateFlash(event, start, flashGeom, color));
  ...
  private animateFlash(event: any, start: number, flashGeom: any, color: Color): void {
    ...
    let strokeColor = color === Color.RED ? `rgba(255, 0, 0, ${opacity})` : `rgba(57, 137, 252, ${opacity})`;
  ...
  public getMarkerFeatureById(markerId: number | string, markerLayer: VectorLayer<any>): Feature | undefined {
