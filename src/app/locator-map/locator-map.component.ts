...
export class LocatorMapComponent implements AfterViewInit, OnDestroy {
  ...
  private addPopup(): void {
    const markerPopup = this.mapService.createOverlay(this.popupService.getPopupOptions('marker-popup'));
  ...
  private handleMapClick(event: any): void {
    ...
    this.distanceService.calculateDistanceOfMarkers(this.markers, event.longitude, event.latitude);
    const sortedMarkers = this.distanceService.sortMarkersByDistance(this.markers);
  ...
  private handleMapHover(event: any, entities: EntityDetails[]) {
    ...
    if (feature && feature.getProperties()['id']) {
      const marker = this.markers.find(m => m.id === feature.getProperties()['id'])!;
      const entity = entities.find(e => e.id === feature.getProperties()['id'])!;
      const flatCoordinates: number[] = (feature.getGeometry() as Point).getFlatCoordinates();
      const coordinates: [number, number] = [flatCoordinates[0], flatCoordinates[1]];
      if (marker) {
  ...
  private handleMarkEntity(entity: any) {
    const markerFeature = this.mapService.getMarkerFeatureById(entity.id, this.markerLayer);
    this.markerService.updateMarkerIcon(markerFeature!, '/assets/marker-icon-selected.png', 48, 48);
